...
class FactScorer(object):
    ...
    def _get_score(self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None):
        ...
        for atom in atomic_facts:
            ...
            if self.lm:
                ...
                output = self.lm.generate(prompt)  # TODO
                # output = 'True'
    ...
    def get_passages(self, topics, atomic_facts=None, knowledge_source=None, verbose=False):
        if knowledge_source is None:
            # use the default knowledge source
            knowledge_source = "enwiki-20230401"
        ...
        assert atomic_facts is not None
        assert len(topics)==len(atomic_facts), "`topics` and `atomic_facts` should have the same length"
        ...
        for topic, facts in zip(topics, atomic_facts):
            for atom in facts:
                passages = self.retrieval[knowledge_source].get_passages(topic, atom['text'], k=5)
                passages = [{'title': psg["title"], 'text': psg["text"].replace("<s>", "").replace("</s>", "")} for psg in reversed(passages)]
                atom['passages'] = passages
                atom['topic'] = topic
...
if __name__ == '__main__':
    ...
    atomic_facts_with_human_labels = []
    ...
    assert args.use_atomic_facts
    out = fs.get_passages(topics=topics,
                          atomic_facts=atomic_facts_with_human_labels,
                          knowledge_source=args.knowledge_source,
                          verbose=args.verbose)

    # TODO: Save out as a json file
    with jsonlines.open(args.input_path.replace(".jsonl", f"_factscore_passages.jsonl"), 'w') as f:
        f.write_all(out)
    exit()
