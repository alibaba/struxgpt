...
model_name_or_path="${MODEL_NAME_OR_PATH:-"./output/MMedBench-CPT-struct-llama2"}"
model_type="${MODEL_TYPE:-"llama2"}"
sft_type="${SFT_TYPE:-"struct"}"
template="${TEMPLATE:-"llama2_sim"}"
...
deepspeed --num_gpus ${ngpus} src/train_bash.py \
    ...
    --dataset mmedbench_sft_${sft_type} \
    --dataset_dir ./data \
    --template ${template} \
    --finetuning_type full \
    --output_dir ./output/MMedBench-SFT-${sft_type}-${model_type} \
    --overwrite_cache \
