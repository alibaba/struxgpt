...
deepspeed="${DEEPSPEED:-"configs/examples/full_multi_gpu/ds_z2_config.json"}"
...
deepspeed --num_gpus ${ngpus} src/train_bash.py \
    ...
    --dataset struxgpt_${version} \
    --dataset_dir ./data \
    --template ${template} \
    --finetuning_type full \
    --output_dir ./output/StruXGPT-${version}-${model_type} \
    --overwrite_cache \
