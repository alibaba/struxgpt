...
class DataArguments:
    ...
    def __post_init__(self):
        if self.dataset_name is not None:
            ...
            subset_flag = info[-1] in ['train', 'test', 'dev', 'val']
            self.dataset_split = info[-1] if subset_flag else 'train'
            self.dataset_name = "/".join(info[:-1]) if subset_flag else '/'.join(info)
