...
def main():
    ...
    encode_loader = DataLoader(
        ...
        # sampler = DistributedSampler(encode_dataset) if torch.cuda.device_count() > 1 else None
    )
    encoded = []
    lookup_indices = []
    ...
    if torch.cuda.device_count() > 1:
        ...
        dp_encoder = nn.DataParallel(model.lm_q)
        model.lm_p = dp_encoder
        model.lm_q = dp_encoder

    # TODO: set lookup_indices to filter unstructurized corpus
            ...
            with torch.cuda.amp.autocast() if training_args.fp16 else nullcontext():
                with torch.no_grad():
                    for k, v in batch.items():
                        batch[k] = v.to(training_args.device) #.to(torch.bfloat16)
                    if data_args.encode_is_qry:
