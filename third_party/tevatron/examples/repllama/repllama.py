...
import logging
from peft import LoraConfig, PeftConfig, get_peft_model, PeftModel, TaskType
...
class RepLLaMA(EncoderModel):
    ...
    def load(
        ...
        base_model = LlamaModel.from_pretrained(config.base_model_name_or_path, torch_dtype=torch_dtype)
        if base_model.config.pad_token_id is None:
            base_model.config.pad_token_id = 0
        logger.info('Loading peft_model...')
        hf_model = PeftModel.from_pretrained(base_model, model_name_or_path, config=config, is_trainable=True, torch_dtype=torch_dtype)
