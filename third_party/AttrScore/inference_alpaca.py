...
def generate_prompt(example, prompt_type, input_has_query=False, demo=None, prompt_key='prompt_input'):
    ...
    if not demo:
        prompt_input = PROMPT_DICT[prompt_key]
        res = prompt_input.format(prompt_template, input)
    else:
        assert prompt_key == 'prompt_input_with_demo'
